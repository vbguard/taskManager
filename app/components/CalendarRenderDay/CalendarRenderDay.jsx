  ...
  const addTask = day => {
    if (calendar) {
      const calendarDay = calendar.find(
        el => el.date === day.format('DD-MM-YYYY'),
      );
        ...
        return (
          <div className="Calendar-tasksWrapper">
            {calendarDay.oneTasks.count !== 0 && (
              <span className="Calendar-oneDayTaskBtn">
                {calendarDay.oneTasks.count}
              </span>
            )}
            {calendarDay.repeatTasks.count !== 0 && (
              <span className="Calendar-repitedTasksBtn">
                {calendarDay.repeatTasks.count}
              </span>
            )}
          </div>
        );
